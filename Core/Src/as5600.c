...
HAL_StatusTypeDef as5600_WriteCommand(I2C_HandleTypeDef *hi2c, uint8_t conf_register_addr, uint8_t setting){
	return HAL_I2C_Mem_Write(hi2c, AS5600_I2C_ADDR, conf_register_addr, 1, &setting, 1, 100);
}
...
HAL_StatusTypeDef as5600_ReadPosition(I2C_HandleTypeDef *hi2c, uint16_t *angle){
	...
	status = HAL_I2C_Mem_Read(hi2c, AS5600_I2C_ADDR, 0x0E, 1, temp, 2, 100);
...
HAL_StatusTypeDef as5600_ReadRawPosition(I2C_HandleTypeDef *hi2c, uint16_t *angle){
	...
	status = HAL_I2C_Mem_Read(hi2c, AS5600_I2C_ADDR, 0x0C, 1, temp, 2, 100);
	*angle = (temp[0] << 8) + temp[1];
...
HAL_StatusTypeDef as5600_ReadConfig(I2C_HandleTypeDef *hi2c, uint16_t *config){
	...
	status = HAL_I2C_Mem_Read(hi2c, AS5600_I2C_ADDR, 0x07, 1, &temp_posision, 1, 100);
	*config = temp_posision << 8;
	if(status == 0){ // jesli pierwszy odczyt ok
		HAL_I2C_Mem_Read(hi2c, AS5600_I2C_ADDR, 0x08, 1, &temp_posision, 1, 100);
...
uint8_t as5600_StatusMagnet(I2C_HandleTypeDef *hi2c){
	...
	HAL_I2C_Mem_Read(hi2c, AS5600_I2C_ADDR, 0x0B, 1, &status, 1, 100);
	return status >> 3;
}
